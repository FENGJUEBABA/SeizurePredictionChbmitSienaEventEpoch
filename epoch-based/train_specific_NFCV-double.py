...
for patient in PATIENTS:
    ...
    for m in range(num):
        for n in range(num):
            if n > m:
                # print(m, '  ', n)
                XZ_XZ.append((XZ[m], XZ[n]))
                XZ_XF.append((XZ[m], XF[n]))
    # X_dict.update({patient: [XZ_XZ, XZ_XF]})
    XX = []
    YY = []
    for i in range(len(XZ_XZ)):
        XX.append(XZ_XZ[i])
        YY.append(1)
        XX.append(XZ_XF[i])
        YY.append(0)
    ...
    num_sample = len(XZ_XZ)
    # 创建网络模型
    model = SiameseNetwork().to('cuda:0')
    #
    input_size = torch.randn(batch_size, C, H, W).to('cuda:0')
    FLOPs, Params = profile(model, inputs=(input_size, input_size))  # 运算量FLOPs,网络参数量 Params
    ...
    # 损失函数
    loss_fn1 = ContrastiveLoss(margin=1)
    ...
    for nf in range(n_fold):  # 交叉验证
        ...
        x_train_filename, y_train, x_test_filename, y_test = nFoldCrossVal(XX[0:num_sample], YY[0:num_sample], n_fold,
                                                                           nf + 1)
        ...
        for i in range(epoch):
            ...
            for j in range(batch_num):
                total_train_step += 1
                data_batch1 = np.zeros((true_batch_size[j], C, H, W))
                data_batch2 = np.zeros((true_batch_size[j], C, H, W))
                for k in range(true_batch_size[j]):
                    data_batch1[k, :] = (np.load(x_train_filename[j * true_batch_size[j] + k][0])).reshape((C, H, W))
                    data_batch2[k, :] = (np.load(x_train_filename[j * true_batch_size[j] + k][1])).reshape((C, H, W))
                x_train_batch1 = torch.Tensor(data_batch1).to('cuda:0')
                x_train_batch2 = torch.Tensor(data_batch2).to('cuda:0')
                y_train_batch = torch.Tensor(y_train[j * batch_size:j * batch_size + true_batch_size[j]]).to('cuda:0')
                y_predict_batch1, y_predict_batch2 = model(x_train_batch1, x_train_batch2)
                y2_train_batch = []
                for it in y_train_batch:
                    if int(it) == 1:
                        y2_train_batch.append([0, 1])
                    else:
                        y2_train_batch.append([1, 0])
                y2_train_batch = torch.Tensor(y2_train_batch).to("cuda:0")
                loss = 0.6 * loss_fn1(y_predict_batch1, y_predict_batch2, y_train_batch) + 0.4 * loss_fn2(
                    y_predict_batch2, y2_train_batch)
                ...
                # 评价指标
                yp1_yp2_cos_score = F.cosine_similarity(y_predict_batch1, y_predict_batch2)
                y_predict_batch1 = y_predict_batch1.cpu().detach().numpy()
                y_predict_batch2 = y_predict_batch2.cpu().detach().numpy()
                yp12_predict_batch = np.argmax(y_predict_batch1, axis=1) * np.argmax(y_predict_batch2, axis=1)
                yp12_predict_batch = torch.Tensor(yp12_predict_batch)
                train_matrix += get_Confusion_Matrix(yp12_predict_batch, y_train_batch)
                ...
                train_batch_auc = get_auc(y_true=y_train_batch, y_score=yp1_yp2_cos_score)
                for b in yp1_yp2_cos_score.to('cpu').detach().numpy():
        ...
        with torch.no_grad():
            ...
            for j in range(batch_num):
                ...
                data_batch2 = np.zeros((true_batch_size[j], C, H, W))
                for k in range(true_batch_size[j]):
                    data_batch1[k, :] = (np.load(x_test_filename[j * true_batch_size[j] + k][0])).reshape((C, H, W))
                    data_batch2[k, :] = (np.load(x_test_filename[j * true_batch_size[j] + k][1])).reshape((C, H, W))
                x_test_batch1 = torch.Tensor(data_batch1).to('cuda:0')
                x_test_batch2 = torch.Tensor(data_batch2).to('cuda:0')
                y_test_batch = torch.Tensor(y_test[j * batch_size:j * batch_size + true_batch_size[j]]).to('cuda:0')
                y_predict_batch1, y_predict_batch2 = model(x_test_batch1, x_test_batch2)
                loss = loss_fn1(y_predict_batch1, y_predict_batch2, y_test_batch)
                ...
                yp12_predict_batch = np.argmax(y_predict_batch1, axis=1) * np.argmax(y_predict_batch2, axis=1)
                yp12_predict_batch = torch.Tensor(yp12_predict_batch)
                test_matrix += get_Confusion_Matrix(yp12_predict_batch, y_test_batch)
                ...
                test_batch_auc = get_auc(y_true=y_test_batch, y_score=yp1_yp2_cos_score)
                for b in yp1_yp2_cos_score.to('cpu').detach().numpy():
