...
data_path = 'D:\learn\dataset\\rawdata\chb-mit'
...
for edfname in File_name:
    temp = mne.io.read_raw_edf(os.path.join(data_path, edfname.split('_')[0][0:5], edfname))
    File_start_abs.append(str(temp.info['meas_date'])[:-6])
    date_temp = str(temp.info['meas_date'])[0:10]
    chbn = edfname.split('_')[0][0:5]
    summary_file = os.path.join(data_path, chbn, chbn + '-summary.txt')
