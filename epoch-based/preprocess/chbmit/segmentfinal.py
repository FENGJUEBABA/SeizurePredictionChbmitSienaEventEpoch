...
DATA_ROOT_PATH = 'D:\learn\dataset\\rawdata\chb-mit'  # chb数据根目录
DATA_SAVE_PATH = 'D:\learn\dataset\segmention\chbmit'  # 处理后的数据保存路径
...
INTERICTAL_BETWEEN_ICTAL = 4 * 60 * 60  # 定义发作间期距离发作期的长度,单位秒，一般4小时
...
txt_file = open('data-summary/chbmit_segmentfinal_summary_generated.txt', 'w')
...
def createclips(clip_dict, type, patients=None):
    ...
    for ii in range(len(keys)):
        if keys[ii][3:5] in patients:
            edf_path = os.path.join(DATA_ROOT_PATH, keys[ii][0:5], keys[ii])
            print('edf_path', edf_path)
            rawEEG = mne.io.read_raw_edf(edf_path, preload=True)
            if set(rawEEG.ch_names) >= set(CHANNELS):  # 当前edf包含所有指定的通道
                ...
                save_path = os.path.join(DATA_SAVE_PATH, keys[ii][0:5], type)
