...
with open('data-summary/configure.txt', "r") as f:
    ...
    for line in lines:
        ...
        if "PREPROCESS_SAVE_PATH" in line:
            PREPROCESS_SAVE_PATH = line.split("=")[1].strip()  # preprocess处理后的数据保存路径
...
def mypreprocess(patientname, over_lop, dur_type):
    ...
    for npy in npys:
        ...
        sample_sum += n
        print(npy)
        condition = seizures['Seizure_filename'].str.startswith(npy.split(SEPARATER)[0])
        seizure_stop_abs_one = seizures[condition]['Seizure_stop_abs'].tolist()
        print(seizure_stop_abs_one)
        seizure_n = ''  # 单个病人第几次发作
        for ssao_i in range(len(seizure_stop_abs_one)):
            if ssao_i == 0:
                if npy_end <= (seizure_stop_abs_one[ssao_i] + POSTICTAL):
                    seizure_n = str(ssao_i)
            else:
                if (seizure_stop_abs_one[ssao_i - 1] + POSTICTAL) < npy_end <= (
                        seizure_stop_abs_one[ssao_i] + POSTICTAL):
                    seizure_n = str(ssao_i)
        if npy_end > (seizure_stop_abs_one[-1] + POSTICTAL):
            seizure_n = 'last'  # 代表最后面的间期数据
        for i in tqdm(range(n)):
            ...
            if int(npy_start + (i * overlop + 1) * WINDOWS) <= npy_end:
                ...
                savepath = os.path.join(PREPROCESS_SAVE_PATH, patientname, dur_type)
                if 'time' in DOMAIN:
                    # 获取时域
                    ...
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    time_file = os.path.join(os.path.join(savepath, 'time_domain'), file_name)
                ...
                if 'fre' in DOMAIN:
                    ...
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    frequency_file = os.path.join(os.path.join(savepath, 'frequency_domain'), file_name)
                ...
                if 'tf' in DOMAIN:
                    ...
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    timefrequency_file = os.path.join(os.path.join(savepath, 'timefrequency_domain'), file_name)
                ...
                if 'mfcc' in DOMAIN:
                    ...
                    mfcc_data = preprocessing.scale(np.abs(mfcc_data).reshape((22, 13 * 198)))
                    if not os.path.lexists(os.path.join(savepath, 'mfcc_domain')):
                        os.makedirs(os.path.join(savepath, 'mfcc_domain'))
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    mfcc_file = os.path.join(os.path.join(savepath, 'mfcc_domain'), file_name)
                ...
                if 'ht' in DOMAIN:
                    ...
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    ht_file = os.path.join(os.path.join(savepath, 'ht_domain'), file_name)
                ...
                if 'hht' in DOMAIN:
                    ...
                    file_name = edf_name + SEPARATER + dur_type + SEPARATER + ns + SEPARATER + ne + SEPARATER + seizure_n + '.npy'
                    hht_file = os.path.join(os.path.join(savepath, 'hht_domain'), file_name)
                    np.save(hht_file, hht_temp)
    if overlop >= 1:
        time_sum = sample_sum * WINDOWS
    else:
        time_sum = sample_sum * overlop * WINDOWS + (1 - overlop) * WINDOWS
    return time_sum
...
def balance_feature_extraction(patients_list):
    ...
    for patient in patients_list:
        Patient_name.append(DATABASE_NAME + patient)
        preictal_size, interictal_size = 0, 0  # 记录发作前期和发作后期数据大小
        patient_path = os.path.join(SEGMENTATION_SAVE_PATH, DATABASE_NAME + patient)
        ...
        if interictal_size > preictal_size:
            ...
            print('间期多倍', patient, '   ', rr)
            interictal_len = mypreprocess(patientname=DATABASE_NAME + patient, over_lop=1,
                                          dur_type='interictal')

            preictal_len = mypreprocess(patientname=DATABASE_NAME + patient, over_lop=OVERLOP,
                                        dur_type='preictal')
            print('{}{}: interictal_len= {}s, preictal_len= {}s'.format(DATABASE_NAME, patient, interictal_len,
                                                                        preictal_len))
        else:
            ...
            interictal_len = mypreprocess(patientname=DATABASE_NAME + patient, over_lop=1,
                                          dur_type='interictal')
            preictal_len = mypreprocess(patientname=DATABASE_NAME + patient, over_lop=OVERLOP, dur_type='preictal')
            print('{}{}: interictal_len= {}s, preictal_len= {}s'.format(DATABASE_NAME, patient, interictal_len,
                                                                        preictal_len))
        Interictal_len.append(interictal_len)
        Preictal_len.append(preictal_len)
    file_csv = pd.DataFrame(
        {'Patient': Patient_name, 'Interictal_len': Interictal_len, 'Preictal_len': Preictal_len})
    file_csv.to_csv('data-summary/time_len_sum.csv', encoding='utf-8', index=None, sep=',')
    print('ALL END')
