...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# The 3D coordinates of MEG sensors are included in the DAN recordings from MEG
...
# `DAN.set_montage <mne.io.Raw.set_montage>`. It is also possible to skip the
...
# raw_1020 = DAN.copy().set_montage(ten_twenty_montage)
# raw_1020 = DAN.copy().set_montage('standard_1020')
...
# ``DAN.info['bads']`` will be plotted in red), and drawing in an existing
...
# same result as mne.channels.find_layout(DAN.info, ch_type='eeg')
